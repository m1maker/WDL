...
bool ListView_GetItem(HWND h, LVITEM *item)
{
  ...
  else 
  {
    ...
    if (mask)
    {
      NMLVDISPINFO nm={{h, (UINT_PTR)GetWindowLong(h,GWL_ID), LVN_GETDISPINFO}};
      nm.item.mask = mask;
...
bool swell_accesskit_get_listview_export_range(HWND hwnd, swell_accesskit_collection_range *range)
{
  ...
  {
    if (active < first || active >= first + count) first = active - count / 2;
    if (first < 0) first = 0;
...
{ 
  ...
  if (g_swell_in_paint)
  {
    printf("swell-generic: calling InvalidateRect() from within paint, this is allowed but bad form.\n");
    // WDL_ASSERT(false);
  }
#endif
